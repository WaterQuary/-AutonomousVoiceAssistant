...
def AI(command):
    response = g4f.ChatCompletion.create(
            ...
            provider=Copilot,
            messages=[
                    {"role": "user", "content": "Ты голосовой помощник по имени Квари. Отвечай коротко, старайся избегать какого то кода или математических формул, только определения и слова." + command}
        ]
    )
...
greetings_morning = [f"Доброе утро {name}."]

greetings_afternoon = [f"Добрый день {name}."]

greetings_evening = [  f"{name} Добрый вечер."]

greetings_night = [f"{name}, доброй ночи"]
